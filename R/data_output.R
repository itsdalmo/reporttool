...
#' Write data to an openxlsx workbook
#'
...
#' x %>% write_sheets(wb, sheet = "test", append = FALSE)
#' openxlsx::saveWorkbook(wb, "test.xlsx", overwrite = TRUE)

write_sheets <- function(df, wb, sheet="analysis", row=1L, append=TRUE) {
...
write_xlsx <- function(lst, file) {
  ...
  lapply(names(lst), function(nm, lst, wb) {
    write_sheets(lst[[nm]], wb = wb, sheet = nm, append = FALSE)}, 
    lst, wb)
