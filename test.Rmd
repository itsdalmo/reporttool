...
# Lowercase columns for reliable referencing.
names(df) <- tolower(names(df))
names(cd) <- tolower(names(cd))
names(hd) <- tolower(names(hd))
names(mm) <- tolower(names(mm))
names(cf) <- tolower(names(cf))

# Set missing values
df[] %<>% sapply(function(x) ifelse(x %in% missingvalues, NA, x))
cd[] %<>% sapply(function(x) ifelse(x %in% missingvalues, NA, x))
hd[] %<>% sapply(function(x) ifelse(x %in% missingvalues, NA, x))
mm[] %<>% sapply(function(x) ifelse(x %in% missingvalues, NA, x))
cf[] %<>% sapply(function(x) ifelse(x %in% missingvalues, NA, x))
