...
if (!is.na(cf$replacement[cf$original == "entity_short"])) {
  entity_short <- str_replace(entity, str_c(" ?", cf$replacement[cf$original == "entity_short"], "[^ ]+ ?"), "")
} else {
  entity_short <- entity
}
...
statement_metric <- "statement_metric" %in% str_to_lower(mm$latent)
statement_categorical <- "statement_categorical" %in% str_to_lower(mm$latent)

# Check variables for open answers in complaints -------------------------------
complaint <- any(c("open_complaint", "open_wouldcomplain") %in% str_to_lower(mm$latent))
open_noncomplaint <- "open_noncomplaint" %in% str_to_lower(mm$latent)
open_wouldcomplain <- "open_wouldcomplain" %in% str_to_lower(mm$latent)
oa_positive <- "oa_positive" %in% str_to_lower(mm$latent)
oa_negative <- "oa_negative" %in% str_to_lower(mm$latent)
open_answer <- "open_answer" %in% str_to_lower(mm$latent)
```

```{r, error=FALSE, results='hide'}
# Convert date columns and get collection dates --------------------------------
df %<>% mutate_each(funs(as.POSIXct(., format = "%m/%d/%Y %H:%M:%S")), startdate, enddate)

survey <- df %>% 
          filter(mainentity == entity) %>%
          summarise(start = format(min(startdate, na.rm=TRUE), "%e. %b. %Y"),
                    end = format(max(startdate, na.rm=TRUE), "%e. %b. %Y"),
                    time = min(startdate, na.rm=TRUE)) %>%
          mutate(month = format(time, "%m"), year = format(time, "%Y")) %>% 
          mutate(period = str_c(ifelse(month <= 6, 
...
if (subentities) {
      valid_sub <- df %>%
                 ...
                 group_by(subentity) %>%
                 summarise(valid = n()) %>%
                 mutate(valid = str_c(subentity, valid, sep = " "))
}
...
lat_info <- mm %>%
            filter(str_to_lower(latent) %in% latents) %>% 
            count(latent) %>%
            summarise(low = min(n), high = max(n), model = sum(n)) %>%
            mutate(total = nrow(filter(mm, str_detect(manifest, "^[qQbBkK]") & 
                                          !str_detect(manifest, "[eE][mM]$"))))
...
- Av de `r respondents$total` inviterte har totalt `r respondents$valid` svart (`r str_c(respondents$valid_percent, "%")`). 
`r if (subentities) str_c("- ", "Svarprosent per ", str_to_lower(cf$replacement[cf$original == "subentity"]), ": ", str_c(valid_sub$res, collapse = ", "))`
- Det er hentet inn svar på `r lat_info$model` spørsmål som bygger opp under hovedanalysen, samt enkelte bakgrunnsspørsmål som bidrar med ytterligere innsikt i forhold til kundeopplevelsen. 
...
- **`r cf$replacement[str_to_lower(cf$original) == "image"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "image"]`)  er målt for å gi en forståelse av respondentenes opplevelse av omdømmet til `r entity`.

- **`r cf$replacement[str_to_lower(cf$original) == "expect"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "expect"]`) er et mål på hva respondentene forventer å få ut av kundeforholdet.

- **`r cf$replacement[str_to_lower(cf$original) == "prodq"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "prodq"]`) fanger respondentens opplevelse av tjeneste- og produkttilbudet.

- **`r cf$replacement[str_to_lower(cf$original) == "servq"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "servq"]`) har fokus på forholdet mellom respondentene og de ansatte hos leverandøren når det gjelder opplevelsen av service og det mer mellommenneskelige.

- **`r cf$replacement[str_to_lower(cf$original) == "value"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "value"]`) er utformet for å fange opp forholdet mellom hva som er oppnådd (levert) og prisen / kostnaden for å få dette, sett fra respondentenes ståsted. 

- **`r cf$replacement[str_to_lower(cf$original) == "epsi"]`** (KTI) er det sentrale målet i modellen, og reflekterer hvorvidt kundene opplever at deres krav og forventninger blir oppfylt. Resultatet for selskapet beregnes på bakgrunn av `r length(mm$manifest[str_to_lower(mm$latent) %in% "epsi"])` spørsmål (`r mm$manifest[str_to_lower(mm$latent) %in% "epsi"]`).

- **`r cf$replacement[str_to_lower(cf$original) == "loyal"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "loyal"]`) måler eksempelvis hvorvidt respondenten ville valgt samme leverandør dersom vedkommende måtte velge på nytt. Forskning viser at lojalitet i stor grad er påvirket av kundetilfredsheten.
...
`r if(subentities == T) str_c("# Profil per", str_to_lower(cf$replacement[cf$original == "subentity" & !is.na(cf$original)]), sep = " ")`
...
pd <- pd %>%
      ...
      bind_rows(mutate(., mainentity = str_c(ent_sum, entity_short, sep = " ")), mutate(., mainentity = subentity)) %>%
      group_by(mainentity) %>%
...
pd <- pd %>% 
      ...
      left_join(cf, by = c("latent" = "original")) %>%
      mutate(latent = factor(replacement, levels=unique(replacement)), ordered = TRUE) %>%
      mutate(mainentity = factor(mainentity, levels = c(str_c(ent_sum, entity_short, sep = " ")), unique(subentity), ordered = TRUE)) %>%
      select(mainentity, latent, score)
...
pd$lines <- ifelse(pd$mainentity == str_c(ent_sum, entity_short, sep = " "), "solid", "dashed")
...
p + geom_line(size=1) +
    geom_point(size=3) +
    scale_color_manual(values = setNames(palette, unique(pd$mainentity))) +
    geom_text(data = filter(pd, mainentity == str_c(ent_sum, entity_short, sep = " ")), 
              aes(label=format(round(score, 1))),  
              size=4, colour="#23373b", vjust=vjust) + 
    ylim(y_min, y_max) + 
...
`r if(historical == T) str_c("# Historisk profil for", entity, sep = " ")`
...
pd <- hd %>%
      ...
      mutate(semester = ifelse(str_to_lower(semester) == "spring", 
                               cf$replacement[cf$original == "spring"], semester)) %>%
      mutate(semester = ifelse(str_to_lower(semester) == "fall", 
                               cf$replacement[cf$original == "fall"], semester)) %>%
      mutate(mainentity = str_c(semester, year, sep = " ")) %>%
...
# Frametitle -------------------------------------------------------------------
if (contrast) {
  cat("##", "Differansen mellom", entity, "og", str_to_lower(ctr_sum) ,"\n", sep=" ")
} else {
  cat("##", "Differansen mellom", entity, "og", str_to_lower(stu_sum) ,"\n", sep=" ")
}

# Get list of manifest names in correct order ----------------------------------
manifests <- data.frame("latent" = latents, stringsAsFactors = FALSE) %>%
             left_join(mutate(mm, latent = str_to_lower(latent)), by = c("latent" = "latent")) %>%
             mutate(question = ifelse(str_length(question) >= 90, str_c(strtrim(question, width=90-3), "..."), question))
...
# Gather the data for the table ------------------------------------------------
pd <- pd %>%
      select(mainentity, one_of(str_c(str_to_lower(manifests$manifest), "em")), w) %>%
      group_by(mainentity) %>%
...
pd <- pd %>%
      ...
      mutate(manifest = str_replace(manifest, "[eE][mM]$", "")) %>%
      left_join(manifests, by = c("manifest" = "manifest")) %>%
      mutate(manifest = str_replace(manifest, "^q", "Q")) %>%
...
pd <- pd %>%
      ...
      mutate(question = str_c(manifest, question, sep = " - ")) %>%
...
text <- ifelse(str_length(text) >= max_length, str_c(strtrim(text, width=max_length-3), "..."), text)
...
first_table <- str_replace_all(first_table, "(\\s|^)(-\\d?\\d\\.\\d*)", "\\1\\\\textcolor[HTML]{D2232B}{\\2}")
second_table <-  str_replace_all(second_table, "(\\s|^)(-\\d?\\d\\.\\d*)", "\\1\\\\textcolor[HTML]{D2232B}{\\2}")
...
`r if(subentities == T) str_c("# Resultat per", str_to_lower(cf$replacement[cf$original == "subentity" & !is.na(cf$original)]), sep = " ")`
...
# Gather the data for the table ------------------------------------------------
pd <- df %>%
      select(mainentity, subentity, one_of(str_c(str_to_lower(manifests$manifest), "em"))) %>%
      filter(mainentity == entity) %>%
      mutate_each(funs(n = as.numeric(.)), -mainentity, -subentity) %>%
      bind_rows(mutate(., mainentity = str_c(ent_sum, entity_short, sep = " ")), mutate(., mainentity = subentity)) %>%
      group_by(mainentity) %>%
...
mvar <- str_replace(mm$manifest[mm$latent == "statement_metric"], "^(q[0-9]{1,2}).?", "\\1") %>%
        na.omit() %>%
        unique()
...
manifests <- mm %>%
             ...
             filter(str_detect(manifest, str_c("^", mvar), ignore.case=TRUE)) %>%
             mutate(manifest = str_to_lower(manifest))
...
mvar <- str_replace(mm$manifest[mm$latent == "statement_categorical"], "^(q[0-9]{1,2}).?", "\\1") %>%
        na.omit() %>%
        unique()
...
manifests <- mm %>%
             ...
             filter(str_detect(manifest, regex(str_c("^", mvar), ignore_case = TRUE), manifest)) %>%
             mutate(manifest = str_to_lower(manifest))
...
p + geom_bar(stat="identity", width=0.5, position=position_dodge(width=0.6)) +
    ...
    geom_text(aes(label=str_c(round(proportion*100, digits=0), "%")),  
...
```{r}
# Frametitle -------------------------------------------------------------------
cat("##", mm$question[str_to_lower(mm$latent) %in% "loyal" & str_detect(str_to_lower(mm$manifest), "15b$")], "\n", sep=" ")
...
mvar <- mm$manifest[str_to_lower(mm$latent) %in% "loyal" & str_detect(str_to_lower(mm$manifest), "15b$")]

# Gather the data --------------------------------------------------------------
pd <- pd %>%
      mutate(recom = pd[[str_to_lower(mvar)]]) %>%
...
# Gather the data for the table ------------------------------------------------
pd <- pd %>%
      mutate(complaint = pd[[str_to_lower(mvar)]]) %>%
...
pd <- df %>%
      select(mainentity, one_of(str_to_lower(mvar))) %>%
      gather(var, answers, -mainentity) %>%
      filter(mainentity == entity & !is.na(answers)) %>%
      mutate(answers = str_replace(answers, "^[ [:punct:]]*", "")) %>%
      mutate(answers = str_c("- ", answers)) %>%
      mutate(nchars = str_length(answers), nlines = ceiling(nchars/135), tlines = cumsum(nlines)) %>%
...
if (nrow(pd) == 0) {
  ...
  pd %>% mutate(answers = str_replace(answers, "^- ", "")) %>%
         select("Svar" = answers) %>%
...
```{r, eval=open_answer}
# Frametitle -------------------------------------------------------------------
frametitle <- str_c("##", mm$question[mm$latent %in% "open_answer"][1], "(uredigerte svar)", sep = " ")
...
```{r, eval=oa_positive}
# Frametitle -------------------------------------------------------------------
frametitle <- str_c("##", mm$question[mm$latent %in% "oa_positive"][1], "(uredigerte svar)", sep = " ")
...
```{r, eval=oa_negative}
# Frametitle -------------------------------------------------------------------
frametitle <- str_c("##", mm$question[mm$latent == "oa_negative" & !is.na(mm$latent)][1], "(uredigerte svar)", sep = " ")
...
if (length(sheets(wb))) {
  saveWorkbook(wb, str_c("../Reports/", entity, " - åpne svar.xlsx"), overwrite = TRUE)
}
```
