...
historical <- entity %in% srv$hd$mainentity
...
- Målgruppen for denne undersøkelsen er kunder/brukere av `r entity`.
...
- Totalt antall svar som ligger til grunne for denne rapporten: `r info$valid`.
...
- Alle resultatene som presenteres i denne rapporten er overlevert for intern bruk i `r entity`.
- EPSIs nasjonale studie av telekom er en separat studie, og resultatene som formidles via EPSIs hjemmesider[^2] kan fritt siteres. 
...
- Indeksen for kundetilfredshet presenteres som en verdi mellom 0 og 100, og er det sentrale målet i disse målingene. Dette er en indeksverdi, ikke en prosentverdi. Jo høyere verdi desto bedre anser kundene deres krav og forventninger oppfylles. 
...
- Som en generell regel kan man si at:
    ...
    - Målinger over 75 anses som høyt/veldig høyt (peker på en sterk relasjon mellom leverandør og kunde). 

- Som en tommelfingerregel så er forskjeller på mer enn 2.5 poeng signifikante. Dette innebærer at dersom det skiller mer enn 2.5 punkter mellom to indeksverdier, så er forskjellen signifikant.
- Se www.epsi-norway.org for mer informasjon om metode og modell, eller om EPSI for øvrig.

## Aspekter i EPSI modellen

- **`r get_translation(srv, "image")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "image")), "og")`) er målt for å fange hvilket inntrykk kundene har, basert på det de har hørt og lest.

- **`r get_translation(srv, "expect")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "expect")), "og")`) er et mål på hva respondentene forventer å få ut av kundeforholdet.

- **`r get_translation(srv, "prodq")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "prodq")), "og")`) fanger respondentens opplevelse av tjeneste- og produkttilbudet.

- **`r get_translation(srv, "servq")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "servq")), "og")`) har fokus på det generelle servicenivået sett fra kundenes ståsted.

- **`r get_translation(srv, "value")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "value")), "og")`) er utformet for å fange opp forholdet mellom hva som er oppnådd (levert) og prisen / kostnaden for å få dette, sett fra respondentenes ståsted. 

- **`r get_translation(srv, "epsi")`** (KTI) er det sentrale målet i modellen, og reflekterer hvorvidt kundene opplever at deres krav og forventninger blir oppfylt. Resultatet for partneren beregnes på bakgrunn av `r length(get_association(srv, "epsi"))` spørsmål (`r conjunct_string(stri_trans_totitle(get_association(srv, "epsi")), "og")`).

- **`r get_translation(srv, "loyal")`** (`r conjunct_string(stri_trans_totitle(get_association(srv, "loyal")), "og")`) måler eksempelvis hvorvidt respondenten ville valgt samme dersom vedkommende måtte velge på nytt. Forskning viser at lojalitet i stor grad er påvirket av kundetilfredsheten.

- Generelt så er det viktig med en balanse mellom forventninger og opplevd kvalitet (selve leveransen). Med andre ord, dersom gapet mellom hva kundene forventer og hvordan de opplever kvaliteten (negativt gap) blir for stor, så vil det ha negativ betydning for tilfredsheten.
...
# Kundeprofil per `r get_translation(srv, "subentity")`
...
pd <- srv %>%
  ...
  mutate(subentity = factor(stri_trans_totitle(subentity))) %>%
  select(-n, -manifest) 

# Get the vertical justification for geom text ---------------------------------
vjust <- pd %>% spread(subentity, answer)
vjust$overlap <- if (ncol(vjust) > 3) rowMeans(vjust[, 2:ncol(vjust)]) else vjust[[ncol(vjust)]]
vjust <- ifelse(vjust[[get_translation(srv, "study_average")]] < vjust[["overlap"]], "2", "-1")
...
# Add linetypes
pd <- pd %>% mutate(line = ifelse(subentity == get_translation(srv, "study_average"), "solid", "dashed"))

# Create the profile-plot ------------------------------------------------------
p <- ggplot(data=pd, aes(x=question, y=answer, group=subentity, colour=subentity, linetype = line))
p + geom_line(size=1) +
  geom_point(size=3) +
  scale_color_manual(values = setNames(palette, rev(unique(pd$subentity)))) +
  geom_text(data = filter(pd, subentity == get_translation(srv, "study_average")), 
            aes(label=sprintf("%.1f", answer)),  
            size=4, colour="#23373b", vjust=vjust) + 
  ylim(y_min, y_max) + 
  guides(linetype=FALSE) +
...
p1 <- p1 + geom_bar(stat="identity", width=0.5, position=position_dodge(width=0.6)) +
  ...
  guides(fill = guide_legend(keywidth = .5, keyheight = .5)) +
  plot_theme() + 
  theme(title = element_text(hjust = 0, size = 12)) +
  ggtitle(get_question(srv, mvar))

# Create the second plot --------------------------------------------------------
mvar <- get_association(srv, "complaint_handling")
call_recode <- lazyeval::interp(
  ~recode(clean_score(x), "Lite fornøyd (1-5)" = 1:5, "Middels (6-8)" = 6:8, "Svært fornøyd (9-10)" = 9:10, as_factor = TRUE), x = as.name(mvar))
...
p2 <- ggplot(pd, aes(x=answer, y=proportion, fill=mainentity, group=mainentity, ymin=0, ymax=1))
...
pd <- srv %>%
  group_by(mainentity) %>%
  survey_table(q19a, q19b, wide = FALSE) %>%
  select(-n) %>%
  filter(mainentity %in% c(entity, get_translation(srv, c("contrast_average", "study_average")))) %>%
  mutate(mainentity = factor(mainentity)) # Drop unused levels
...
p + geom_bar(stat="identity", width=0.5, position=position_dodge(width=0.6)) +
    scale_fill_manual(values=setNames(palette, levels(pd$mainentity)), drop=FALSE) +
    facet_wrap(~ question, ncol=1, scales="free_x") +
    scale_x_discrete(labels = function(x) simple_wrap(x, 15)) + 
    scale_y_continuous(labels=percent) +
    geom_text(aes(label=sprintf("%.0f%%", proportion*100)),  
              size=3, colour="#23373b", position=position_dodge(width=0.6), vjust=-1.1, hjust=.35) +
    ...
    theme(strip.background = element_blank(),
          strip.text = element_text(color = "#23373b", hjust = 0, size = 12),
          panel.margin = unit(2, "lines"))

```

```{r} 
cat("##", get_question(srv, "q19a"), "\n", sep = " ")

# Gather the data
pd <- srv %>%
  group_by(mainentity, q19a) %>%
  survey_table(epsi, loyal, wide = FALSE) %>%
  select(-n) %>%
  filter(mainentity %in% c(entity, get_translation(srv, c("contrast_average", "study_average")))) %>%
  mutate(mainentity = factor(mainentity)) # Drop unused levels

# Plot the data
p <- ggplot(pd, aes(x=q19a, y=answer, fill=mainentity, group=mainentity, ymin=0, ymax=100))
p + geom_bar(stat="identity", width=0.5, position=position_dodge(width=0.6)) +
    facet_wrap(~ question, ncol=1, scales="free_x") +
    scale_fill_manual(values=setNames(palette, levels(pd$mainentity)), drop=FALSE) +
    scale_x_discrete(labels = function(x) simple_wrap(x, 15)) + 
...
pd <- get_data(srv) %>%
  ...
  mutate(page = if (max(tlines) > 23) cut(tlines, breaks = ceiling(sum(nlines)/23), labels = FALSE) else 1L)
