...
- **`r cf$replacement[str_to_lower(cf$original) == "image"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "image"]`)  er målt for å gi en forståelse av hvordan respondentene oppfatter at `r entity` vurderes av andre som har kjennskap til barnehagen. 
...
- **`r cf$replacement[str_to_lower(cf$original) == "servq"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "servq"]`) har fokus på forholdet mellom respondentene og de ansatte når det gjelder opplevelsen av service og det mer mellommenneskelige.
...
- **`r cf$replacement[str_to_lower(cf$original) == "epsi"]`** (KTI) er det sentrale målet i modellen, og reflekterer hvorvidt kundene opplever at deres krav og forventninger blir oppfylt. Resultatet for barnehagen beregnes på bakgrunn av `r length(mm$manifest[str_to_lower(mm$latent) %in% "epsi"])` spørsmål (`r mm$manifest[str_to_lower(mm$latent) %in% "epsi"]`).

- **`r cf$replacement[str_to_lower(cf$original) == "loyal"]`** (`r mm$manifest[str_to_lower(mm$latent) %in% "loyal"]`) måler eksempelvis hvorvidt respondenten ville valgt samme barnehage dersom vedkommende måtte velge på nytt. Forskning viser at lojalitet i stor grad er påvirket av kundetilfredsheten.
...
```{r, eval=complaint}
# Frametitle -------------------------------------------------------------------
cat("##", mm$question[mm$latent %in% c("open_complaint", "open_wouldcomplain")][1], "\n", sep=" ")
...
pd <- df %>%
      ...
      filter(mainentity == entity & !is.na(answer)) %>%
