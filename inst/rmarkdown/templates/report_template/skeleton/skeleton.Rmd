...
- Av de `r respondents$total` inviterte har totalt `r respondents$answered` svart (`r paste0(respondents$valid_percent, "%")`). 
...
- Indeksen for kundetilfredshet presenteres som en verdi mellom 0 og 100, og er det sentrale målet i disse målingene. Dette er en indeksverdi, ikke en prosentverdi. Jo høyere verdi desto bedre anser kundene (de foresatte) at barnehagen oppfyller deres krav og forventninger. 
...
- **NB**: Ved nedbrytninger på avdelingsnivå er tallgrunnlaget begrenset. På dette nivået bør resultatene tolkes med en viss grad av forsiktighet, da hvert enkelt svar har større vekt og ekstremverdier kan gi tildels store utslag.
...
manifests <- data.frame("latent" = latents, stringsAsFactors=F) %>%
             ...
             mutate(question = ifelse(str_length(question) >= 90, 
                                  paste0(strtrim(question, width=90-3), "..."), question))
...
pd[[cf$replacement[cf$original == "question"]]] <- 
...
```{r}
# Frametitle
cat("##", mm$question[mm$latent == "complaint" & !is.na(mm$latent)], "\n", sep=" ")
...
```{r, results='hide'}
# Frametitle
frametitle <- paste("##", mm$question[mm$latent == "open_complaint" & !is.na(mm$latent)][1], "(uredigerte svar)")
...
# Frametitle
frametitle <- paste("##", mm$question[mm$latent == "open_wouldcomplain" & !is.na(mm$latent)][1], "(uredigerte svar)")
...
# Frametitle
frametitle <- paste("##", mm$question[mm$latent == "open_noncomplaint" & !is.na(mm$latent)][1], "(uredigerte svar)")
...
```{r, results='hide'}
# Frametitle
frametitle <- paste("##", mm$question[mm$latent == "oa_positive" & !is.na(mm$latent)][1], "(uredigerte svar)")

# Variable
mvar <- mm$manifest[mm$latent == "oa_positive" & !is.na(mm$latent)]
...
pd %>% mutate(openanswer = gsub("^- ", "", openanswer)) %>% 
       ...
       write.csv2(paste0("Reports/", entity, " - positive åpne svar", ".csv"), fileEncoding="latin1")
```
...
# Variable
mvar <- mm$manifest[mm$latent == "oa_negative" & !is.na(mm$latent)]
...
pd %>% mutate(openanswer = gsub("^- ", "", openanswer)) %>% 
       select("Apne svar" = openanswer) %>%
       write.csv2(paste0("Reports/", entity, " - negative åpne svar", ".csv"), fileEncoding="latin1")
```
