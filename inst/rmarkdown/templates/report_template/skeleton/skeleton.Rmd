...
- Totalt antall svar som ligger til grunn for denne rapporten: `r info$valid`.
...
pd <- pd %>%
  ...
  mutate(page = if (max(tlines) > 23) cut(tlines, breaks = ceiling(sum(nlines)/23) + 1L, labels = FALSE, right = FALSE) else 1L)
