...
test_that("recode works with dplyr/magrittr pipe", {
  ...
  y <- x %>% 
       ...
       mutate_each(funs(recode(., "Great" = . <= 100, "Average" = . <= 75, "Bad" = . < 60, as_factor = TRUE)), score, score2)
...
test_that("recode works for integer (and returns a factor if wanted)", {
  
  x <- c(1, 2, 3)
  y <- recode(x, "test" = . %in% 1:3, as_factor = TRUE)
